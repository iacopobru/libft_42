...
/*   Updated: 2024/12/08 22:17:37 by ibrunial         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char **gen_test_string()
{
	...
	if (!str_arr)
	{
		printf("[MALLOC ERROR] an allocation error occur in gen_test_string\n");
		return (NULL);
	}
	...
	**str_arr = '\0';
	for (int i = 0; i < NSTR; i++){
		...
		for (int j = 0; j < L_str - 1; j++){
			str_arr[i][j] = " abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"[rand() % 63];
		}
		str_arr[i][L_str - 1] = '\0';
	}
	return (str_arr);
...
char	**copy_test_str(char **test_str)
{
	...
	if (!cp_test_str)
	{
		printf("[MALLOC ERROR] an allocation error occur in copy_test_string\n");
		return (NULL);
	}
	for (int i = 0; i < NSTR; i++)
	{
